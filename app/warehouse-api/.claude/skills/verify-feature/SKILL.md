...
3. **Lấy access token nếu route cần JWT** — gọi `POST /api/{VERSION}/auth/login` bằng tài khoản root (`ROOT_PHONENUMBER`/`ROOT_PASSWORD` trong `.env`, mặc định `root`/`root`) để có token role `SUPER_ADMIN`, dùng test route giới hạn bởi `@HasRoles(...)`. Nếu spec yêu cầu test route theo role thấp hơn (`CUSTOMER`...), tự đăng ký thêm 1 user qua `POST /api/{VERSION}/auth/register` rồi login bằng user đó.
...
4. **Gọi lần lượt từng route trong mục "API cần có" của spec** bằng `curl` (hoặc tool HTTP có sẵn), đối chiếu:
   ...
   - Role không đủ quyền → bị chặn đúng (gọi lại route create/update/delete bằng token role thấp hơn bảng phân quyền trong spec, phải nhận lỗi 401/403, không được lọt qua).
