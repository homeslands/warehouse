...
- `POST /materials` **bắt buộc** `baseUnitSlug` (`MATERIAL_BASE_UNIT_SLUG_IS_REQUIRED`, chặn cả thiếu/`null`/rỗng) — vật tư mới luôn có đơn vị cơ sở. Cột `base_unit_id_column` vẫn NULL-able chỉ vì dữ liệu cũ, và **không có cách gỡ về NULL qua PATCH** (`pickDefined` lọc cả `null`) — vật tư cũ tạo trước migration `1783728000020` vẫn chưa có đơn vị cơ sở, và chưa endpoint nào gán được tỉ lệ khi vật tư chưa khai base unit.
