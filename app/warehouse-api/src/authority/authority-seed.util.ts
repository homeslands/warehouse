...
/**
 * Dùng chung cho mọi migration seed `Authority` (mỗi feature mới = 1 migration gọi 2 hàm dưới đây).
 * Cố tình KHÔNG đặt trong `src/migrations/`: glob `migrations/*{.ts,.js}` nạp mọi export ở đó làm
 * migration.
 */
...
export async function seedAuthorities(
  ...
  seeds: readonly AuthoritySeed[],
): Promise<void> {
  const groupIdByName = new Map<string, string>();
  for (const groupName of new Set(seeds.map((item) => item.group))) {
...
export async function unseedAuthorities(
  queryRunner: QueryRunner,
  seeds: readonly AuthoritySeed[],
): Promise<void> {
  const codes = seeds.map((authority) => authority.code);
  ...
  const groupNames = [...new Set(seeds.map((item) => item.group))];
