        ...
        if (err) {
          if (err instanceof FileException) {
            if (err.code) {
              switch (err.code) {
                case 121010:
                  return reject(new FileException(FileValidation.MUST_EXCEL_FILE));
              }
            }
          }
          ...
          if (err instanceof multer.MulterError) {
            switch (err.code) {
              case 'LIMIT_FILE_SIZE':
                return reject(new FileException(FileValidation.FILE_SIZE_EXCEEDS_LIMIT_ALLOWED));
              case 'LIMIT_UNEXPECTED_FILE':
                return reject(new FileException(FileValidation.LIMIT_UNEXPECTED_FILE));
              case 'LIMIT_PART_COUNT':
                return reject(new FileException(FileValidation.LIMIT_PART_COUNT));
              case 'LIMIT_FIELD_KEY':
                return reject(new FileException(FileValidation.LIMIT_FIELD_KEY));
              case 'LIMIT_FIELD_COUNT':
                return reject(new FileException(FileValidation.LIMIT_FIELD_COUNT));
              case 'LIMIT_FIELD_VALUE':
                return reject(new FileException(FileValidation.LIMIT_FIELD_VALUE));
          ...
          return reject(new FileException(FileValidation.ERROR_WHEN_UPLOAD_FILE));
        }
        resolve(next.handle());
      });
    });
